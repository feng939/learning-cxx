...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        for (unsigned int i = 0; i < size; ++i) {
            unsigned int idx[4] = { (i / strides[3]) % shape[0], (i / strides[2]) % shape[1], (i / strides[1]) % shape[2], i % shape[3] };
            unsigned int other_idx[4] = { idx[0] % others.shape[0], idx[1] % others.shape[1], idx[2] % others.shape[2], idx[3] % others.shape[3] };
            unsigned int other_flat_idx = other_idx[0] * other_strides[3] + other_idx[1] * other_strides[2] + other_idx[2] * other_strides[1] + other_idx[3];
